...
def parse_line(line: str, datetime_format: str) -> Tuple[Optional[datetime], str, str]:
    ...
    if line_elements:
        ...
        try: 
            ...
            print(f"applying .strptime() to: {message_datetime}, expecting datetime format {exception_datetime_format}") # %m/%d/%y, %H:%M
            timestamp = datetime.strptime(message_datetime, exception_datetime_format) # %m/%d/%y, %H:%M:%S
